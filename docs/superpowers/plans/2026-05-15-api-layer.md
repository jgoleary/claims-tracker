...
**Goal:** Working FastAPI application exposing all routes from the spec, wired to the
backend core from Plan 1, with TestClient-based integration tests.

**Architecture:** FastAPI with sync SQLAlchemy via `get_db` dependency injection. Alert
computation in `alerts.py` (pure function, no DB access). Automation subprocess runner
with `data/state.json` persistence. Frontend communicates with this server on port 8000.

**Tech Stack:** FastAPI, SQLAlchemy 2.0 (sync), Pydantic v2, httpx (TestClient), Python
3.11+

**Builds on:** Plan 1 backend core — all imports from `app.models`, `app.matching`,
`app.ingest`, `app.storage`, `app.database`, `app.config` are available.
...
FastAPI API layer is complete. The server starts with
`cd backend && source .venv/bin/activate && uvicorn app.main:app --reload` and serves all
endpoints on `http://localhost:8000`.

**Next:** `docs/superpowers/plans/2026-05-15-frontend.md` — React + TypeScript + Vite +
Tailwind frontend with all 8 pages.

**Known gaps to address in Plan 3 or as follow-up:**

- `GET /matches/suggestions` uses a Python-side suggestion loop — acceptable, but should
  validate the logic is consistent with `run_matching()`
- `ingest_benefits` route accepts any key (not just in_network/out_of_network) — validated
  by Pydantic schema now via `BenefitsIngest`
