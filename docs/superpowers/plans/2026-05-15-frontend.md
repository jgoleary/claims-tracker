...
**Goal:** Working React frontend with all 8 pages wired to the FastAPI backend, runnable with `npm run dev` on port 5173.

**Architecture:** Vite + React 18 + TypeScript + Tailwind CSS v3. TanStack Query v5 for all server state. React Router v6 for routing. No component library — Tailwind utilities only. Vite proxy forwards `/api` to `localhost:8000`.

**Tech Stack:** React 18, TypeScript 5, Vite 5, Tailwind CSS 3, TanStack Query 5, React Router 6

**No automated tests in v1.** Verification = `npm run build` passes (TypeScript + bundle) and the dev server starts cleanly.

**Builds on:** Plan 2 FastAPI API — all endpoints available at `http://localhost:8000/api`.
...
**Files:** `src/types.ts`, `src/api.ts`, `src/utils.ts`, `src/components/Layout.tsx`, `src/components/Alert.tsx`
...
const FLAG_ORDER = ['MISSING', 'DENIED', 'STALE_PENDING', 'UNDERPAID', 'APPROVED_ZERO_PAID']
        ...
        <div className="space-y-2">
          {visibleAlerts.map((alert, i) => (
            <div key={i} className="flex items-center gap-4 bg-white border rounded-lg px-4 py-3 shadow-sm">
              ...
              <span className="text-xs text-gray-400">{Object.entries(alert.details).map(([k, v]) => `${k}: ${v}`).join(' · ')}</span>
              <Link
                to={`/submissions/${alert.submission_id}`}
...
function AddModal({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const body = { ...form, amount_billed: form.amount_billed * 100, expected_reimbursement: form.expected_reimbursement * 100 }
      ...
      <div className="space-y-3">
        ...
        {(
          ...
          <div key={key}>
            ...
            <input type={type} value={form[key] as string} onChange={set(key)}
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <input placeholder="Filter by member…" value={filterMember}
              ...
              {(data ?? []).map((sub) => (
                <tr key={sub.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link to={`/submissions/${sub.id}`} className="font-medium text-blue-600 hover:underline">
                      {sub.member_name}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3 text-gray-700">{formatCents(sub.amount_billed)}</td>
                  <td className="px-4 py-3 text-gray-700">{formatCents(sub.expected_reimbursement)}</td>
    ...
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => { if (confirm('Delete this submission?')) deleteMutation.mutate() }}
        ...
        <div className="bg-white border rounded-lg p-6 shadow-sm">
          ...
          <dl className="space-y-3 text-sm">
            {[
              ...
              ['Network', sub.network_treatment === 'in_network_exception' ? 'In-Network Exception' : 'Out-of-Network'],
            ].map(([label, value]) => (
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => updateMutation.mutate()}
                    className="px-3 py-1.5 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">Save</button>
                  <button onClick={() => setEditing(false)}
                    className="px-3 py-1.5 text-gray-600 text-sm hover:text-gray-800">Cancel</button>
                </div>
              </div>
            ) : (
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm text-gray-600 flex-1">{sub.notes || <span className="text-gray-400 italic">No notes</span>}</p>
                <button onClick={() => { setNotes(sub.notes ?? ''); setEditing(true) }}
          ...
          {sub.pdf_path && (
            <div className="mt-4 pt-4 border-t">
              <a href={api.submissions.pdfUrl(sub.id)} target="_blank" rel="noreferrer"
                className="text-sm text-blue-600 hover:underline">Download PDF →</a>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border rounded-lg p-6 shadow-sm">
          ...
          {!claim ? (
            <div className="text-gray-400 text-sm py-8 text-center">
              {sub.anthem_claim_number ? 'Loading claim…' : 'Not matched to an Anthem claim yet.'}
              {!sub.anthem_claim_number && (
                <div className="mt-2">
                  <Link to="/matches" className="text-blue-600 hover:underline text-sm">Review suggestions →</Link>
                </div>
              )}
            </div>
          ) : (
              ...
              <div className="grid grid-cols-2 gap-0 divide-x">
                ...
                <div className="p-5">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <div className="text-gray-600">Billed: {formatCents(s.submission.amount_billed)} · Expected: {formatCents(s.submission.expected_reimbursement)}</div>
                  </div>
                </div>
                {/* Candidates */}
                <div className="p-5">
            ...
            <tbody className="divide-y divide-gray-100">
              {(data ?? []).map((c) => (
                <tr key={c.claim_number} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs ${c.is_matched ? 'text-green-600' : 'text-gray-400'}`}>
                      {c.is_matched ? '✓' : '—'}
                    </span>
                  </td>
                </tr>
    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white border rounded-lg p-6 shadow-sm mb-6">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Launches the Playwright script to fetch your latest claims and benefits from Anthem.
          Chromium will open — log in and complete MFA if prompted.
        </p>
      ...
      <div className="bg-white border rounded-lg p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-500 mb-4">
          If the automation script isn't available, export the CSV from Anthem manually and upload it here.
        </p>
        <input
          type="file"
          accept=".csv"
        ...
        {csvError && (
          <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded text-sm text-red-700">{csvError}</div>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-white border rounded-lg p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Learned mappings between your provider names and Anthem's names. Created automatically when you confirm a match.
        </p>

        {isLoading ? (
          <div className="text-gray-500 text-sm">Loading…</div>
        ) : !aliases?.length ? (
          <div className="text-gray-400 text-sm">No aliases yet. They'll appear here after you confirm matches.</div>
        ) : (
          <table className="w-full text-sm">
      ...
      <div className="mt-6 bg-white border rounded-lg p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-500 mb-3">
          Configured in <code className="bg-gray-100 px-1 rounded text-xs">backend/app/config.py</code>. Edit that file and restart the server to change thresholds.
        </p>
...
**Next:** `docs/superpowers/plans/2026-05-15-playwright.md` — Playwright automation scripts (`fetch_claims.py`, `fetch_benefits.py`, `fetch_all.py`).
