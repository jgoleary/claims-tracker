...
the two dev servers directly. First set up a virtualenv with the dev dependencies and
...
> `deploy/install.sh` re-creates `backend/.venv` with runtime-only deps via `uv`, so
> run the above again if you've installed the always-on service in the same checkout.
