...
- **`alerts.py`** — `compute_flags(submission, match)` returns a list of `Alert` dataclasses. Thresholds live in `config.py` (MISSING_DAYS=30, STALE_PENDING_DAYS=45, UNDERPAID_MIN_CENTS=$25, UNDERPAID_PCT=10%). `APPROVED_ZERO_PAID` flag is suppressed when `expected_reimbursement == 0`.
